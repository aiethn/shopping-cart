...
function Cart() {
  ...
  const code = useSelector((state) => state.shipping.value);
  const [totalCost, setTotalCost] = useState(0);
  const [showModal, setShowModal] = useState(false);
  const [goBayar, setGoBayar] = useState(false);

  const shipCost = 10;

  useEffect(() => {
    if (code === "BELANJAGRATIS") {
      setTotalCost(cost);
    } else {
      setTotalCost(cost + shipCost);
    }
  }, [code, cost]);

  return (
    <div className="flex flex-col space-y-4 bg-white rounded-lg border border-gray-200 shadow-md w-full p-4">
      <p className="font=bold text-2xl mb-4">The total amount of</p>
      ...
      <div className="flex justify-between">
        <p>Shipping</p>
        <p>{code === "BELANJAGRATIS" ? "FREE" : `$${shipCost}`}</p>
      </div>
      <div className="flex justify-between">
                  ...
                  {!goBayar ? (
                    <div>
                      <div className="text-center">
                        <p className="text-lg">The total you have to pay</p>
